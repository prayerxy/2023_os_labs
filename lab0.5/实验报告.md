...
跳转至0x8000后，开始运行`bootloader`，负责开机并加载操作系统至内存中。
...
可以看到VIRT_DRAM的地址空间是从0x80000000开始的，也就是说qemu加载bootloader程序到了0x80000000。



**然后**便是opensbi的底层初始化，实现了`hart id`的判断、清除寄存器的值、清除bss段、设置sp栈指针读取设备树、fbt重定位和跳转`sbt_init`的操作。通过底层初始化，完成了底层的配置，然后便可以进入到sbi正式的程序。



**再**便进行opensbi的设备初始化，而这便是为==二级boot的跳转==做准备，在设备初始化的最后便是准备下一级boot的跳转地址，其实也便是操作系统内核加载的最终地址。例如opensbi在qemu-riscv-64和qemu-riscv-32的两种环境下最终的跳转地址都是`0x80000000 + 0x200000`，即`0x80200000`。



**最后**经过二级boot的跳转便进入了操作系统内核加载的地址，便可以实现操作系统内核的加载。
