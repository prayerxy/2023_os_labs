...
alloc_proc(void) {
     ...
     */
     //LAB5 YOUR 2113665 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
     */
    //LAB6 YOUR 2113665 : (update LAB5 steps)
    /*
     ...
     */
     ...
     //LAB8 YOUR 2113665 : (update LAB6 steps)
      /*
     * below fields(add in LAB6) in proc_struct need to be initialized
...
proc_run(struct proc_struct *proc) {
    if (proc != current) {
        ...
        */
       //LAB8 2113665 : (update LAB4 steps)
      /*
     * below fields(add in LAB6) in proc_struct need to be initialized
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    //:EXERCISE2 2113665  HINT:how to copy the fs in parent's proc_struct?
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
  ...
  //LAB5 2113665 : (update LAB4 steps)
   /* Some Functions
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2113665  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
 ...
 /* (1) create a new mm for current process 
    ...
    //phnum是程序头的数量  这里从fd对应的文件读取elf文件头  phoff是elf文件头的偏移量  position of program header
    for (phnum = 0; phnum < elf->e_phnum; phnum ++) {
        ...
        while (start < end) {
            ...
            //页为单位读取，可能不足一页  offset = ph->p_offset
    ...
    //argc是命令行参数数量，kargv是指向命令行参数数组，每个字符串是一个命令行参数
    for (i = 0; i < argc; i ++) {
