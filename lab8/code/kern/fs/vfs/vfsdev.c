...
//把设备device与inode联通起来，然后把设备信息加入到vdev_list中
//文件系统可通过链接vfs_dev_t结构的双向链表找到device对应的inode数据结构    即devnode
//这样一个inode节点的成员变量in_type的值是0x1234，则此inode的成员变量in_indo是device结构
typedef struct {
    const char *devname;
