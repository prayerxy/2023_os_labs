...
sfs_do_mount(struct device *dev, struct fs **fs_store) {
    ...
    void *sfs_buffer; //缓冲区大小 分配
    if ((sfs->sfs_buffer = sfs_buffer = kmalloc(SFS_BLKSIZE)) == NULL) {
        goto failed_cleanup_fs;
    }

    /* load and check superblock */
    //从这个设备对应的磁盘读取超级块内容到sfs_buffer
    ...
    uint32_t freemap_size_nbits = sfs_freemap_bits(super);//总块数
    //创建一个bitmap 设置sfs文件系统的freemap
    if ((sfs->freemap = freemap = bitmap_create(freemap_size_nbits)) == NULL) {
        goto failed_cleanup_hash_list;
    }
    uint32_t freemap_size_nblks = sfs_freemap_blocks(super);//占多少块  也就是freemap的位数
    //从磁盘块块号2读取 bitmap至freemap.map
