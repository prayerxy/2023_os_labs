...
disk0_io(struct device *dev, struct iobuf *iob, bool write) {
    ...
    uint32_t blkno = offset / DISK0_BLKSIZE;//偏移的磁盘块的个数 
    uint32_t nblks = resid / DISK0_BLKSIZE; //读写多少个磁盘块
    ...
    lock_disk0();
    //无论读写，都需要一个中间的缓冲区disk0_buffer
    while (resid != 0) {
        size_t copied, alen = DISK0_BUFSIZE;
        if (write) {
            //将数据从iob->base移动到disk0_buffer
            iobuf_move(iob, disk0_buffer, alen, 0, &copied);//真实读写的数据长度
            assert(copied != 0 && copied <= resid && copied % DISK0_BLKSIZE == 0);
            nblks = copied / DISK0_BLKSIZE;
            disk0_write_blks_nolock(blkno, nblks);//从disk0_buffer写到磁盘扇区
        }
        else {
            if (alen > resid) {
                alen = resid;
            ...
            disk0_read_blks_nolock(blkno, nblks);  //从磁盘扇区读到disk0_buffer
            iobuf_move(iob, disk0_buffer, alen, 1, &copied); //从disk0_buffer读到iob->iobase
...
disk0_device_init(struct device *dev) {
    ...
    //初始化dev
    dev->d_blocks = ide_device_size(DISK0_DEV_NO) / DISK0_BLK_NSECT;///所有磁盘的扇区个数/一个磁盘块的扇区个数
