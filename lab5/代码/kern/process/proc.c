...
set_links(struct proc_struct *proc) {
    ...
    //younger pointer
    proc->yptr = NULL;
    //设置older pointer以及younger pointer之间关系 
    if ((proc->optr = proc->parent->cptr) != NULL) {
        proc->optr->yptr = proc;
    }
    //其中一个进程的cptr永远是最小的孩子节点指针
...
//复制用户进程的mm，设置为Proc的mm
static int
copy_mm(uint32_t clone_flags, struct proc_struct *proc) {
    struct mm_struct *mm, *oldmm = current->mm;

    /* current is a kernel thread */
    //proc->mm描述的是用户态空间，所以内核态为null
    if (oldmm == NULL) {
        return 0;
    }
    //根据 clone_flags 的值，决定是复制还是共享内存管理结构
    if (clone_flags & CLONE_VM) {
        //共享，将当前进程的mm复制给新进程的mm
        mm = oldmm;
        goto good_mm;
    }
    int ret = -E_NO_MEM;
    //表示clone_flags不含CLONE_VM，需要复制内存管理结构，新建mm
    ...
    //为上文新建的mm进行复制，复制的内容包括内存空间的映射关系，以及内存空间的内容
    //加锁的目的是为了保护对内存管理结构的并发访问。
    lock_mm(oldmm);
    {   
        //将父进程mm的vma链表完全复制在子进程mm的vma链表中
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    */
    ...
    //nr_process++;
    //list_add(&proc_list, &(proc->list_link));
    //update step5 设置关系链 young older child pointer
    set_links(proc);
    // 设置进程状态
...
//   2. 将进程的状态设置为 PROC_ZOMBIE，然后调用 wakeup_proc(parent) 来要求父进程回收自己
//   3. 调用调度器以切换到其他进程
...
do_exit(int error_code) {
    ...
    local_intr_save(intr_flag);
    {
        ...
        while (current->cptr != NULL) {
            ...
            //现它是最新的孩子节点，所以older pointer为NULL
            proc->yptr = NULL;
 ...
 */
//被do_execve调用，完成加载放在内存中的执行程序到进程空间，这涉及到对页表等的修改，分配用户栈
static int
load_icode(unsigned char *binary, size_t size) {
    //mm为空
    //加载应用程序，需要创建一个新的mm，用于描述进程的用户态空间
    if (current->mm != NULL) {
        panic("load_icode: current->mm must be empty.\n");
    }

    int ret = -E_NO_MEM;
    //创建一个新的mm，用于描述进程的用户态空间  
    struct mm_struct *mm;
    ...
    //(3.4) find every program section headers
        ...
        //一般p_filesz <= p_memsz，如果不是，说明程序有问题
    ...
    //(3.5) call mm_map fun to setup the new vma ( ph->p_va, ph->p_memsz)
        ...
        size_t off, size;
        //映射段的虚拟地址  p_va
        //它指定了程序段在虚拟地址空间中的位置。当操作系统加载 ELF 文件时，会根据程序段的信息。
        //将每个程序段映射到相应的虚拟地址空间中，以便程序在执行时可以正确访问和执行其中的代码和数据。
...
repeat:
    haskid = 0;
    if (pid != 0) {
        //如果 pid 不为 0，则查找指定的子进程，并检查其父进程是否为当前进程。
        proc = find_proc(pid);
        ...
        //如果 pid 为 0，则查找当前进程的子进程，检查是否有子进程处于僵尸状态。
        proc = current->cptr;
    ...
    //如果存在子进程，但其状态不为 PROC_ZOMBIE，则将当前进程的状态设置为 PROC_SLEEPING
    //等待子进程退出。同时，设置当前进程的等待状态为 WT_CHILD，表示当前进程正在等待子进程退出。
    if (haskid) {
    ...
    return -E_BAD_PROC;
//进行子进程退出的处理，包括回收子进程的内核栈和进程控制块所占的内存空间。
found:
    ...
    local_intr_restore(intr_flag);
    //释放子进程的内核栈空间和进程控制块（proc_struct）所占用的内存。
    put_kstack(proc);
    kfree(proc);
    return 0;
}
...
//给一个进程设置PF_EXITING标志（“kill”信息，即要它死掉），这样在trap函数中，将根据此标志，让进程退出
int
do_kill(int pid) {
    struct proc_struct *proc;
...
kernel_execve(const char *name, unsigned char *binary, size_t size) {
 ...
 //在这里，%1 表示立即数操作数，它的值由内联汇编代码中的 "i"(SYS_exec) 提供。SYS_exec 是一个常量，用于表示系统调用号。
    asm volatile(
        //将寄存器a0的值赋值为sys_exec的系统调用号
        "li a0, %1\n"
        "lw a1, %2\n"
        "lw a2, %3\n"
        "lw a3, %4\n"
