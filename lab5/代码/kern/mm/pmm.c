...
//用于释放与线性地址la相映射的物理地址的Page结构，并清除与线性地址la相关联的pte
...
void unmap_range(pde_t *pgdir, uintptr_t start, uintptr_t end) {
    ...
    do {
        //获取线性地址 start 对应的页表项 0表示不创建pte
        pte_t *ptep = get_pte(pgdir, start, 0);
        if (ptep == NULL) {
            //如果页表项不存在，说明这段内存区域没有被映射，直接跳过
...
void exit_range(pde_t *pgdir, uintptr_t start, uintptr_t end) {
    //确保 start 和 end 是以页大小（PGSIZE）对齐的，并且位于用户空间。
    assert(start % PGSIZE == 0 && end % PGSIZE == 0);
    assert(USER_ACCESS(start, end));
    ...
    do {
        ...
        if (pde1&PTE_V){
            ...
            do {
                ...
                if (pde0&PTE_V) {
                    ...
                    //只有一级页表的所有页表项都无效，才会释放pde二级页表项映射的一级页表的页
                    if (free_pt) {
            ...
            //只有三级页表的d1start对应的页表项，映射的二级页表中所有页表项都无效，才会把二级页表的页释放
            if (free_pd0) {
        ...
        d1start += PDSIZE;
        //三级页表的前一个页表项对应的一个二级页表被处理完，此时d0start就是d1start
        d0start = d1start;
 ...
 * 其中copy_mm包含share、dup两种方式，dup方式调用dup_mmap，dup_mmap调用copy_range
 */
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end,
               bool share) {
    //确保传入start和end是以页大小对齐的，并且位于用户空间
    assert(start % PGSIZE == 0 && end % PGSIZE == 0);
    assert(USER_ACCESS(start, end));
    ...
    do {
        ...
        if (ptep == NULL) {
            // if pte is NULL, just skip it 跳到下一个页表项应该映射的页的线性地址
            start = ROUNDDOWN(start + PTSIZE, PTSIZE);
            continue;
        }
...
int page_insert(pde_t *pgdir, struct Page *page, uintptr_t la, uint32_t perm) {
    ...
    if (*ptep & PTE_V) {  //如果PTE_V，说明原先存在映射，不然应该全为0，所以现在先删除原先的映射
        ...
        if (p == page) {//如果原先映射的物理页面就是page，那么直接更新页表项
            page_ref_dec(page);//之前加1了，现在恢复
        } else {//如果原先映射的物理页面不是page，那么先删除原先的映射，再更新页表项
            page_remove_pte(pgdir, la, ptep);
        }
    }
