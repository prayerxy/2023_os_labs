...
static void page_init(void) {
    ...
    pages = (struct Page *)ROUNDUP((void *)end, PGSIZE); //实际为虚拟地址
    ...
    mem_begin = ROUNDUP(freemem, PGSIZE);  // mem_begin是freemem向上取整的结果
    mem_end = ROUNDDOWN(mem_end, PGSIZE);
    if (freemem < mem_end) {
        //pa2page()将mem_begin物理地址转换为对应Page结构体的地址
        //init_memmap()调用pmm->init_memmap()来初始化我们可以使用的物理内存
